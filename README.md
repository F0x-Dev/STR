...
**CRITICAL**: Before deploying to production, read **[SECURITY.md](SECURITY.md)** for essential security requirements.

**Quick Security Checklist:**
- Generate secure `JWT_SECRET`: `openssl rand -hex 32`
- Set strong `ADMIN_PASSWORD` (minimum 16 characters)
- Create `.env` from `.env.example` with secure values
- Configure SSL certificates for HTTPS
- Review firewall rules and restrict port access
- **Never commit `.env` file to git**

**The application will refuse to start with default/weak credentials for your safety.**
...
- Multi-bitrate transcoding pipeline using FFmpeg (360p, 720p, 1080p)
...
- Stream key validation (alphanumeric only)
- Mandatory secure credential validation on startup
- Network isolation (separate frontend/backend/monitoring networks)

### Monitoring & Observability
- Prometheus metrics export with custom stream metrics
...
- Database connection pooling (async SQLite)
...
STREAM_KEY=$(openssl rand -base64 32 | tr -dc 'a-zA-Z0-9' | head -c 32)
...
- `ADMIN_PASSWORD` - Set a strong password (16+ characters)
